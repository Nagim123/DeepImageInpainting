...
class Trainer:
    def __init__(self, model: Module, train_loader: DataLoader, val_loader: DataLoader, loss_fn, ) -> None:
        pass
...
def train_one_epoch_with_discriminator(generator, discriminator, train_loader, loss_fn, opt_gen, opt_disc, device):
    ...
    for i, batch in progress:
        ...
        loss_gen = (loss_fn(output, torch.ones_like(output)) + loss_fn(fake, target))
        loss_gen.backward()
        opt_gen.step()
